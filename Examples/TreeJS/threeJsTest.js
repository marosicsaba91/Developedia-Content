...
// Airplane
fbxLoader.load(planeUrl, onLoad);
let airPlane;
...
function onLoad(object) {
  ...
  const airplaneMaterial = new T.MeshStandardMaterial({color: 0xffffff});
...
function update() {
  ...
  const rotationStep = clock.getDelta() * angleRad;
  box.rotation.y += rotationStep;

  if (airPlane != null)
    airPlane.rotation.y += rotationStep;
